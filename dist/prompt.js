...
export function isInterestingWord(word) {
...
export function selectPromptFromPosts(posts) {
...
export function processPrompt(post) {
    if (!post.title) {
    ...
    const numBlanks = Math.min(Math.max(3, Math.floor(words.length / 6)), // At least 3, or 1 per 6 words
...
export function createGamePrompt(processedPrompt) {
    return [
