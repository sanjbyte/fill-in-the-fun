...
export async function startNewGame(context) {
    try {
        ...
        const gameState = createGameState(subreddit.name, topPost.id, topPost.body, blankedText, blankWords, blankIndices, topPost.authorName ?? "Unknown");
        ...
        await context.ui.showToast({
            text: `Error starting the game: ${error instanceof Error ? error.message : "Unknown error"}`,
        });
    }
}
